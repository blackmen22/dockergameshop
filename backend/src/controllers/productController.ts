...
export class ProductController {
  ...
  static addProduct(req: Request, res: Response, next: NextFunction) {
    ...
    Product.create(product, (err: Errback, result: any) => {
      if (err) {
        res.status(500).json({ status: "product not added", message: err });
      } else {
        res.json({
          status: "success",
    ...
    Product.find()
      ...
      .exec((err: Errback, result: any) => {
        ...
        Product.find({ category: category }, (err: Errback, result: any) => {
          if (err) {
            res.status(500).json({ status: "product not founded", message: err });
          } else {
            res.json({
              status: "success",
              message: "Products Found",
  ...
  static updateProduct(req: Request, res: Response, next: NextFunction) {
    Product.findByIdAndUpdate(
      ...
      (err: Errback, result: any) => {
        if (err) {
          res.status(500).json({ status: "product not updated", message: err });
        } else {
          res.json({
            status: "success",
    ...
    Product.find()
      ...
      .exec((err: Errback, result: any) => {
        ...
        Product.find(
          ...
          (err: Errback, result: any) => {
            if (err) {
              res.status(500).json({ status: "We can't found product list", message: err });
            } else {
              res.json({
                status: "success",
