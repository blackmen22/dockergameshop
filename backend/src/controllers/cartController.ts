...
export class CartController {
  static getUserCart(req: Request, res: Response, next: NextFunction) {
    Cart.aggregate(
      ...
      (err: Errback, result: any) => {
        if (err) {
          res.status(500).json({ status: "error user cart", message: err });
        } else {
  ...
  static cardSaver(req: Request, res: Response, next: NextFunction) {
    const wishlist = new Cart(req.body);
    Cart.create(wishlist, (err: Errback, result: any) => {
      if (err) {
        res.status(500).json({ status: "not working", message: err });
      } else {
        res.json({
          status: "success",
  ...
  static updateCart(req: Request, res: Response, next: NextFunction) {
    Cart.updateMany(
      ...
      (err: Errback, result: any) => {
        ...
        if (err) {
          res.status(500).json({ status: "mission imposible", message: err });
        } else {
          res.json({
            status: "success",
